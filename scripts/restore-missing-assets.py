...
and for each missing file whose original URL points to e-glaser.de:

  1. Locates the asset in local import directories.
  2. Copies it to  public/img/<subpath>.
  3. Rewrites the original URL in the Markdown source files.
...
# Destination directory inside public/ for restored assets
UPLOADS_DIR = PUBLIC_DIR / "img"

# Hostnames whose missing assets should be searched for locally
SITE_HOSTS = {"e-glaser.de", "www.e-glaser.de"}

# Search order for local asset files
SEARCH_ROOTS = [
...
def url_to_subpath(url: str) -> str | None:
    """Extract the path-relative subpath from a URL, or None if not applicable."""
