...
USER_AGENT = (
    "Mozilla/5.0 (compatible; nuxt-link-checker/1.0; "
    "+https://github.com/Elektro-Glaser-GmbH/elektro-glaser-gmbh.github.io)"
)
...
def check_internal(url: str) -> tuple[str | None, int]:
    ...
    if path.startswith('/blog/'):
        ...
        if not slug:
            return done(None)
        matches = list((CONTENT_DIR / 'blog').rglob(f"*{slug}*.md"))
        if not matches:
